   ...
   <head>
    ...
    $(function () {
        $('#selectyear').on('submit', function (e) {
          ...
          $.ajax({
            type: 'POST',
            url: 'ajax/get_student_year.php',
            data: $(this).serialize(),
            success: function(data){
              var myRecords = JSON.parse(data);
              var student = "";
              if (myRecords.success==1){
                for (i=0;i<myRecords.student.length;i++) {
                  var myRecord = myRecords.student[i];
                  var newstudent = "<option value='"+myRecord.studentID+"'>"+myRecord.firstName+" "+myRecord.lastName+"</option>";
                  student = student+newstudent;
                }
                document.getElementById("students").innerHTML = student;
             }
             else{
               document.getElementById("students").innerHTML = "<h>There are no students in this class</h>";
             }
            }
          });
        });
      ...
      $(function () {
          $('#selectstudent').on('submit', function (e) {
            e.preventDefault();

            $.ajax({
              type: 'POST',
              url: 'ajax/get_student_topic_engagement.php',
              data: $(this).serialize(),
              success: function(data){
                ...
                function init() {
                  ...
                  if (myRecords.success==1){
                    for (i=0;i<myRecords.pin.length;i++) {
                      ...
                      var description = myRecord.taskName+', '+myRecord.date;
      ...
      <div class="row row-bottom-padded-md">
        <div class="ui raised text container segment">
          ...
          <div class="fluid three ui buttons">
            <button name='topic' id='topic' class="grey medium ui button"><a href='viewEngagementTopic.php'>Topic<a></button>
            <button name='class' id='class' class="grey medium ui button"><a href='viewEngagementClass.php'>Class</a></button>
            <button name='student' id='student' class="medium ui button"><a href='viewEngagementStudent.php'>Student</a></button>
          </div>
           ...
           <div id='div2' class="ui form segment" hidden>
             <form id='selectstudent'>
               <div class="form-group">
                 Student <select id="students" name="students" class="fluid ui search dropdown"></select>
               </div>
               <button class="ui fluid primary submit button" name="submit_button2" id="submit_button2" type="submit">Select Student</button>
             </form>
           </div>
           <canvas id = "canvas1" width="1000" height="600" hidden></canvas>
           <style>
           canvas {
                padding: 0;
                margin: auto;
